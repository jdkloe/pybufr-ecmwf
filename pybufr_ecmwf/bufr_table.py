            ...
            if f_val == 3:
                ...
                if self.bufr_table_set.verbose:
                    print('adding expanded version of: %6.6i' %
                          descr.reference)
                tmp_list = (self.bufr_table_set.table_d[descr.reference].
                            expand())
                ...
                for j in range(yyy):
                    for repl_descr in descr_list_to_be_replicated:
                        ...
                        if repl_descr_f_val == 3:
                            ...
                            expanded_descriptor_list.extend(
                                repl_descr.expand())
                        else:
                            if self.bufr_table_set.verbose:
            ...
            if f_val == 3:
                ...
                if yyy == 0:
                    ...
                    for j in range(yyy):
                        for repl_descr in descr_list_to_be_replicated:
                            repl_descr_f_val = (
                                int(repl_descr.reference/100000.))
                            if repl_descr_f_val == 3:
                                if self.verbose:
                                    print(('%i: adding expanded version ' +
                                           'of: %6.6i') %
...
#            raise IOError
        ...
        for (i, line) in enumerate(bfd.readlines()):
            ...
            if len(line_copy) >= 118:
                ...
                if self.report_warnings:
                    ...
                    print("You could report this to " +
    ...
    def custom_c_split(self, line):
        ...
        if line[12:16].strip() == '':
            ...
            try:
                ...
                remaining_parts = line[12:].split(None, 2)
            except TypeError:
                import string
    ...
    def add_c_table_entry(self, lineblock):
        ...
        flag_value = None
        text = ''
        num_text_lines_added = 0
        # ?? this will not work; todo: sort out what is needed here
        num_text_lines = 0
    ...
    def load_d_table(self, dfile):
        ...
        if self.verbose:
            ...
            # print('Loaded {} D table entries'.
            #       format(len(self.table_d.keys())))
    ...
    def read_WMO_csv_table_b(self, b_filename):
        ...
        if (nr_of_ignored_probl_entries > 0):
            print("nr_of_ignored_probl_entries = " +
                  str(nr_of_ignored_probl_entries))
        print("Loaded: "+str(len(table_b))+" table B entries")
        print("-------------")
    ...
    def add_to_B_table(self, descriptor):
        #  #[
        assert(isinstance(descriptor, Descriptor) is True)
    ...
    def add_to_D_table(self, descriptor):
        #  #[
        assert(isinstance(descriptor, CompositeDescriptor) is True)
    ...
    def write_C_table(self, fd):
        ...
        for ref in sorted(self.table_c):
            ...
            for i, flag_value in (
                   enumerate(sorted(self.table_c[ref].flag_dict))):
...
if __name__ == "__main__":
    ...
    def check_tablefile(bufr_table):
        ...
        if realname_bufr_table in handled_orig_names:
            return
